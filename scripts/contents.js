...
// 初始化动作计数和历史记录
let dragCount = parseInt(localStorage.getItem('dragCount')) || 0;
let engineSelectCount = parseInt(localStorage.getItem('engineSelectCount')) || 0;
...
// 更新动作统计显示
function updateActionStatsDisplay() {
    document.getElementById('drag-count').textContent = dragCount;
    document.getElementById('engine-select-count').textContent = engineSelectCount;
...
function updateHistoryDisplay() {
    ...
    historyRecords.forEach(record => {
        ...
        listItem.style.padding = '2px 0'; // 紧凑排列
...
document.addEventListener('dragend', (event) => {
    ...
    if (selectedText) {
        dragCount += 1;
        localStorage.setItem('dragCount', dragCount);
        historyRecords.push(`拖动: ${selectedText}`);
        localStorage.setItem('historyRecords', JSON.stringify(historyRecords));
        updateActionStatsDisplay();
...
document.querySelectorAll('.select-button').forEach(button => {
    button.addEventListener('click', () => {
        engineSelectCount += 1;
        localStorage.setItem('engineSelectCount', engineSelectCount);
        updateActionStatsDisplay();
    });
});
...
document.getElementById('search-button').addEventListener('click', () => {
    ...
    if (searchText) {
        ...
        historyRecords.push(`搜索: ${searchText}`);
        localStorage.setItem('historyRecords', JSON.stringify(historyRecords));
        updateActionStatsDisplay();
        updateHistoryDisplay();
    }
